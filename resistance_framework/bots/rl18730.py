...
class PlayerRecord(object):
    ...
    def single_round_padded_and_masked_sus_tuple(self, round_num: int = -1) -> NN_INPUTS:
        ...
        lead_sus: List[Tuple[int, float, int]] = self.mission_teams_lead_sus_levels
        been_sus: List[Tuple[int, float, int]] = self.mission_teams_been_on_sus_levels
        for_sus: List[Tuple[int, float, int]] = self.approved_teams_sus_levels_and_suspect_count
        against_sus: List[Tuple[int, float, int]] = self.rejected_teams_sus_levels
        for_not_on_sus: List[Tuple[int, float, int]] = self.sus_levels_of_non_hammer_teams_approved_whilst_not_on
        ...
        votes_s_c: Tuple[int, Tuple[int, int, int, int, int, int, int], Tuple[int, int, int, int, int, int, int]] =\
            list(
                reversed(
                    self.per_round_counts_of_teams_voted_for_and_against_with_suspect_counts
                    )
                )[0]
