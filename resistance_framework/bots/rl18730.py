...
"""
A TypeAlias for the tuple holding the data generated by a player for a single round
that will be put into the neural network (for just that player)
"""

NN_5INPUTS = Tuple[NN_INPUTS, NN_INPUTS, NN_INPUTS, NN_INPUTS, NN_INPUTS]
"""
A tuple holding 5 NN_1INPUTS.
This is basically the data for all the players in a single timestep,
and is pretty much the per-timestep size of data that the neural network expects to get.
"""

NN_T_INPUTS = Tuple[NN_5INPUTS, ...]
"""
...
class RoleAllocationEnum(Enum):
    ...
    def extract_sublist_from(self, input_list: List[T]) -> List[T]:
        ...
        """
        return list(input_list[p] for p in range(0, len(input_list)) if self.value[p])
...
class GamestateTree(object):
    ...
    def get_hammer_indices(cls) -> List[int]:
        ...
        """
        return list(k for k in cls._node_dict.keys() if cls._node_dict[k].hammer)
...
class TempTeamRecord(object):
    ...
    def generate_teamrecord_from_data(self) -> TeamRecord:
        ...
        """
        return TeamRecord(
            self.team, self.leader, self.mission_number, self.nomination_attempt,
...
class PlayerRecord(object):
    ...
    @property
    def sus_levels_of_approved_non_hammer_teams_approved_whilst_not_on(self) -> List[Tuple[int, float, int]]:
        """
        Relative suspicions from teams that this player voted for, whilst not being on aforementioned team.
        Grants leniency for hammer teams (as resistance members are forced to vote in favour of hammer teams)
        """
    ...
    @property
    def per_round_counts_of_teams_voted_for_and_against_with_suspect_counts_normalized(self) -> List[
        Tuple[
            int,
        ...
        :return: A list of tuples containing
            * round ID
            * tuple with the normalized number of times the agent has voted for a team with those suspect counts
                (indexed to suspect count, 0-5, with all 5+ suspect counts sent to index 5)
    ...
    def hammers_thrown(self) -> List[int]:
        ...
        return list(v for v in self._teams_rejected if v in hammer)
    ...
    def simple_spy_probability(
        ...
        if any(True for h in self.hammers_thrown if h in all_prior_rounds) > 0:
    ...
    T_props = TypeVar(
        ...
        Tuple[int, Tuple[float, float, float, float, float, float], Tuple[float, float, float, float, float, float]]
    )
    """
    ...
    def trim_property_list_to_before_round_n(self, prop_list: List[T_props], round_n: int = -1) -> List[T_props]:
        ...
        if type(prop_list[0]) == int:
            ...
            # are in prior_rounds
            return list(p for p in prop_list if p in prior_rounds)
        else:
            ...
            return list(p for p in prop_list if p[0] in prior_rounds)

    _default_sus_tuple: ClassVar[NN_INPUTS] = (
    ...
    def get_padded_and_masked_sus_lists_multiple_rounds_for_player_record_trainer(
        ...
        or 0.0 (no count). All values will be between 0 and 1.
        """
        ...
        lead_sus: List[Tuple[int, float, int]] = \
            list(d for d in self.mission_teams_lead_sus_levels if d[0] in priors)
        been_sus: List[Tuple[int, float, int]] = \
            list(m for m in self.mission_teams_been_on_sus_levels if m[0] in priors)
        for_sus: List[Tuple[int, float, int]] = \
            list(f for f in self.approved_teams_sus_levels_and_suspect_count if f[0] in priors)
        against_sus: List[Tuple[int, float, int]] = \
            list(a for a in self.rejected_teams_sus_levels if a[0] in priors)
        for_not_on_sus: List[Tuple[int, float, int]] = \
            list(n for n in self.sus_levels_of_approved_non_hammer_teams_approved_whilst_not_on if n[0] in priors)

        hammers_thrown: List[int] = list(h for h in self.hammers_thrown if h in priors)

        votes_s_c: List[
            Tuple[int, Tuple[float, float, float, float, float, float], Tuple[float, float, float, float, float, float]]
        ] = self.per_round_counts_of_teams_voted_for_and_against_with_suspect_counts_normalized[0: len(priors)+1]
    ...
    def single_round_padded_and_masked_sus_tuple(self, round_num: int = -1) -> NN_INPUTS:
        ...
        if len(for_sus) + len(against_sus) == 0:
            return PlayerRecord._default_sus_tuple
        ...
        return (
            ...
            sum(b[1] for b in been_sus) / len(been_sus) if len(been_sus) > 0 else 0.5,
            sum(f[1] for f in for_sus) / len(for_sus) if len(for_sus) > 0 else 0.5,
            sum(a[1] for a in against_sus) / len(against_sus) if len(against_sus) > 0 else 0.5,
            sum(n[1] for n in for_not_on_sus) / len(for_not_on_sus) if len(for_not_on_sus) > 0 else 0.5,
            len(list(h for h in self.hammers_thrown if h in priors))/5,
            *votes_s_c[1],
            *votes_s_c[2]
        )
...
class GameRecord(object):
    ...
    """
    A dedicated logger for all gamerecord-related info.
    Such as the data that would be fed into the NN.
    """
    if not log.handlers:
        try:
            log.addHandler(logging.FileHandler(resources_file_path/"GameRecord.log"))
    ...
    def get_info_about_sabotages_from_spies_for_sabotage_records(self,
        ...
        for m in (kv for kv in self._team_records.items() if kv[1].sabotages != -1):
    ...
    def get_prior_round_indices(self, round_n: int = -1) -> List[int]:
        ...
        if round_n == -1 or round_n >= len(indices):
            return indices
        else:
    ...
    def belief_state_json_string(self) -> str:
        ...
        out_dict = {
            ...
            # 18 floats in innermost tuple, 5 tuples in outer tuple, 1 outer tuple per teamrecord
            "hsd_out": [],
            #  List[Tuple[float, ... ]] 5 in the tuple, 1 tuple per teamrecord
            "spies": tuple(1 if p in self._spies else 0 for p in self._player_records.keys())
            #  Tuple[int, int, int, int, int]
        }
        round_no: int = 0
    ...
    def log_json_beliefs(self) -> NoReturn:
        """
        Call this to log the JSON belief info stuff held in this GameRecord to the GameRecord log file.
        :return:
        """
...
class rl18730(Bot):
    ...
    def vote(self, team: List[TPlayer]) -> bool:
        ...
        if is_hammer:
            if self.spy:
                ...
                elif self.game.wins == 2 and not any(True for p in team if p in self.spies):
    ...
    def onVoteComplete(self, votes: List[bool]) -> NoReturn:
        ...
        self.temp_team_record.add_vote_info(
            list(p for p in self.game.players if votes[p.index]),
            sum(votes) >= 3
        )
    ...
    def say(self, message: str) -> NoReturn:
        ...
        # EDIT: the important data is now on the __name__ log, which (should) be in bots/rl18730/GameRecord.log.
        # so I can now use this log to be annoying :^)
        pass
    ...
    def onGameComplete(self, win: bool, spies: Set[TPlayer]) -> NoReturn:
        ...
        self.game_record.log_json_beliefs()
