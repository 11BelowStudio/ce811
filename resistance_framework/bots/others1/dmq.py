...
class RuleStatistics:
    ...
    def __repr__(self):
        ...
        for key, value in sorted(iter(self.entries.items()), key=lambda k_v6: (k_v6[1],k_v6[0].index)):
...
class ResistanceBaseBehavior:
    ...
    def __cmp__(self, other):
        assert isinstance(other, ResistanceBaseBehavior) or issubclass(other, ResistanceBaseBehavior)
...
class OneSpySelectionBehavior(ResistanceBaseBehavior):
    ...
    def process(self, game, owner, phase):
        ...
        sorted_by_trust = []
        for key, value in sorted(iter(owner.entries.entries.items()), key=lambda k_v7: (k_v7[1],k_v7[0].index)):
            if not(key in spies):
...
class OneSpyLessSuspiciousSelectionBehavior(ResistanceBaseBehavior):
    ...
    def process(self, game, owner, phase):
        if game.turns > 1 and game.wins == 0:
            spies = list(owner.memory.spies)
            sorted_by_trust = []
            for key, value in sorted(iter(owner.entries.entries.items()), key=lambda k_v: (k_v[1],k_v[0].index)):
                if not(key in spies):
                    sorted_by_trust.append((key, value))
            sorted_by_trust.reverse()
...
class LessSuspiciousSelectionBehaviour(ResistanceBaseBehavior):
    ...
    def process(self, game, owner, phase):
        sorted_by_trust = []
        for key, value in sorted(iter(owner.entries.entries.items()), key=lambda k_v8: (k_v8[1],k_v8[0].index)):
            if key != owner:
                sorted_by_trust.append((key, value))
        sorted_by_trust.reverse()
...
class NotSelectedSelectionBehaviour(ResistanceBaseBehavior):
    ...
    def process(self, game, owner, phase):
        ...
        if game.turn == 3 and game.wins == 0:
            sorted_by_trust = []
            for key, value in sorted(iter(owner.entries.entries.items()), key=lambda k_v1: (k_v1[1],k_v1[0].index)):
                if key != owner:
                    sorted_by_trust.append((key, value))
            sorted_by_trust.reverse()
...
class MostUntrustedSelectionBehaviour(ResistanceBaseBehavior):
    ...
    def process(self, game, owner, phase):
        ...
        if game.turn == 2 and game.wins == 0:
            sorted_by_trust = []
            for key, value in sorted(iter(owner.entries.entries.items()), key=lambda k_v2: (k_v2[1],k_v2[0].index)):
                if key != owner:
                    sorted_by_trust.append((key, value))
            sorted_by_trust.reverse()
...
class ResistanceMemberBasicVoting(ResistanceBaseBehavior):
    ...
    def process(self, game, owner, phase):
        ...
        for key, value in sorted(iter(owner.entries.entries.items()), key=lambda k_v9: (k_v9[1],k_v9[0].index)):
            if key != owner:
                sorted_by_trust.append((key, value))
...
class NotSelectedOnVotingBehaviour(ResistanceBaseBehavior):
    ...
    def process(self, game, owner, phase):
        ...
        if game.turn == 3 and game.wins == 0:
            sorted_by_trust = []
            for key, value in sorted(iter(owner.entries.entries.items()), key=lambda k_v3: (k_v3[1],k_v3[0].index)):
                if key != owner:
                    sorted_by_trust.append((key, value))
            #sorted_by_trust.reverse()
            #if sorted_by_trust[0][1] <= 0.5:
...
class NotSelected3OnVotingBehaviour(ResistanceBaseBehavior):
    ...
    def process(self, game, owner, phase):
        ...
        if game.turn == 3 and game.wins == 0:
            sorted_by_trust = []
            for key, value in sorted(iter(owner.entries.entries.items()), key=lambda k_v4: (k_v4[1],k_v4[0].index)):
                if key != owner:
                    sorted_by_trust.append((key, value))
            sorted_by_trust.reverse()
            if sorted_by_trust[0][1] <= 0.5:
...
class NotSelected2OnVotingBehaviour(ResistanceBaseBehavior):
    ...
    def process(self, game, owner, phase):
        ...
        if game.turn == 2 and game.wins == 0:
            sorted_by_trust = []
            for key, value in sorted(iter(owner.entries.entries.items()), key=lambda k_v5: (k_v5[1],k_v5[0].index)):
                if key != owner:
                    sorted_by_trust.append((key, value))
            sorted_by_trust.reverse()
            if sorted_by_trust[0][1] <= 0.5:
...
class ResistanceMemberBCTeamMembers(ResistanceBaseBehavior):
    ...
    def process(self, game, owner, phase):
        sorted_by_trust = []
        for key, value in sorted(iter(owner.entries.entries.items()), key=lambda k_v10: (k_v10[1],k_v10[0].index)):
            if key != owner:
                sorted_by_trust.append((key, value))
        sorted_by_trust.reverse()
...
class OnMissionCompletedResistanceFailBehavior(ResistanceBaseBehavior):
    """One spy in the team"""
    def process(self, game, owner, phase):
        if owner.memory.lastSabotage is not None and owner.memory.lastSabotage > 0:
