...
class InferenceBot(StatBot):
    ...
    def _allpairs(self):
        """ All possible pairs. """
        res = []
        others = sorted([p for p in self.players if p.index!=self.index], key=lambda p: p.index)
    ...
    def _leastSpyish(self, players, count):
        team = sorted([(self.sprobs[p], p) for p in players
                       if (p not in self.spies and p not in self.safe)], key=lambda t:t[0])
        s = list(self.safe)
        if len(s) >= count:
            return s[:count]
        else:
