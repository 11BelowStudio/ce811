...
from player import Player, Bot, TPlayer
import random
...
class Trusty(Bot):
    ...
    def __init__(self, game, index, spy):
        ...
        super(Trusty, self).__init__(game, index, spy)
        self.leader = 0
        self.players = {}
        self.turnhistory = []
        self.spies = []
    ...
    def select(self, players, count):
        ...
        if self.spy:
            ...
            if random.random() < self.stochasticity:
                ...
                others = sorted([(self.players[p.index].spyness(), p)
                          for p in players if p.index != self.index], key=lambda p: self.players[p[1].index].spyness())

                res = [p for (_, p) in others][:count-1]
        return me+ res

    def vote(self, team) -> bool:
        return self._vote(team, self.game.leader, self.game.tries)

    def _vote(self, team, leader, tries):
    ...
    def onVoteComplete(self, votes) -> None:
        return self._onVoteComplete(self.game.players, votes, self.game.team)

    def _onVoteComplete(self, players, votes, team):
    ...
    def onMissionComplete(self, sabotaged: int) -> None:
        return self._onMissionComplete(self.game.team, sabotaged)

    def _onMissionComplete(self, selectedPlayers, sabotaged):
    ...
    def onGameComplete(self, win: bool, spies) -> None:
        self._onGameComplete(self.game.players, spies)
