...
class SoreLooser(Bot):
    ...
    def __init__(self, game, index, spy):
        super(SoreLooser, self).__init__(game, index, spy)
...
#            return me + random.sample(others, count-1)
    ...
    def _onMissionComplete(self, team, sabotaged):
    ...
    def _sabotage(self, team):
        ...
        if len(team) == 3:
            if self.accomplice in team and self.accomplice.index > self.index:
                return False
            else:
                return True
