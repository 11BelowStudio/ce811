...
class Rogue(Bot):
    ...
    def __init__(self, game, index, spy):
        super(Rogue, self).__init__(game, index, spy)
        self.spy = spy
        self.index = index
        pass
...
# If you're a spy, this function will be called to list the spies, including others and yourself.
    ...
    def vote(self, team):
        return self.own_vote(team, self.game.leader, self.game.tries)

    def own_vote(self, team, leader, tries):
    ...
    def onMissionComplete(self, sabotaged: int) -> None:
        self.ownOnMissionComplete(self.game.team, sabotaged)

    def ownOnMissionComplete(self, team, sabotaged):
